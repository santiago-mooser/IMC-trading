...
class RegressionStrategy(Strategy):
    def __init__(self, name: str, min_req_price_difference: int, max_position: int, intercept: float, coef: float):
        super().__init__(name, max_position)
        ...
        self.mm = StoikovMarketMaker(intercept, coef, 0, 20)
        # self.logger = FloatListLogger('./features.txt')
    def trade(self, trading_state: TradingState, orders: list):
        order_depth: OrderDepth = trading_state.order_depths[self.name]
        ...
        if len(self.prices) == self.strategy_start_day:
            ...
            avg_bid, avg_ask = fair_price - 1, fair_price + 1
        else:
            avg_bid, avg_ask =  self.calculate_prices(self.strategy_start_day)
        self.mm.update_inventory(self.prod_position)
    ...
    def calculate_imbalance(self, order_depth: OrderDepth) -> float:
        ...
        if bid_vol + ask_vol == 0:
            return 0
        imbalance = (bid_vol + ask_vol)/ (bid_vol - ask_vol)
        # self.logger.log([bid_vol, ask_vol, imbalance])
        return imbalance     
...
class Starfruit(RegressionStrategy):
    def __init__(self, intercept, coef):
        super().__init__("STARFRUIT", min_req_price_difference=2.5, max_position=20, intercept = intercept, coef = coef)
...
class Trader:
    ...
    def __init__(self, intercept, coef) -> None:
        self.products = {
            "STARFRUIT": Starfruit(intercept, coef),
            "AMETHYSTS": Amethysts()
        }
        self.logger = Logger()
...
class FloatListLogger:
    def __init__(self, file_path):
        """
        Initializes the logger with a path to the text file where the data will be stored.

        Args:
        file_path (str): The path to the file where the list of floats will be logged.
        """
        self.file_path = file_path

    def log(self, float_list):
        """
        Logs a list of floats to a text file, appending each new list on a new line.

        Args:
        float_list (List[float]): The list of floats to log.
        """
        # Convert each float to string and join them with a space
        line = ' '.join(f"{x:.6f}" for x in float_list) + '\n'
